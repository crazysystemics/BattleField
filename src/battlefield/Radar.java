...
public class Radar {
    ...
    public Radar(int id, CartesianLocation rLoc, double pri){
        ...
        MIN_SCAN_ELEVATION = 0 + (MAX_BEAM_HEIGHT/2);
        MAX_SCAN_ELEVATION = PHI - (MAX_BEAM_HEIGHT/2);
    ...
    public String getStatus() {
        ...
        status  = status + xk_1.getSphLoc().getRho() + "," + xk_1.getSphLoc().getDPhi() + "," + xk_1.getSphLoc().getDTheta();
        return status;
    }
    public void scan() {
        if (EM.getIsEchoMatrixSet() == 0) {
            if (reverseAzimuthalScan == 1) {
                radarAntennaAzimuthalDirection = radarAntennaAzimuthalDirection - 7.5;
            }
    ...
    public void setEchoMatrix(AirCraft Ac, Radar R) {
        EM.setEchoMatrix(Ac, R);
    }
